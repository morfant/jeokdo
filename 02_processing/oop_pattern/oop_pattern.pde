...
중간중간 이전에 나온 개념들에 대해
이건 뭐였고 이건 뭐였지.. 라는 떠올려 보는 것도 좋구요.

ex) 누군가 function이 뭔가요? 하고 물었을 때,
그 정의를 답해줄 수 있는 것은 지금의 상태에서는 중요치 않다.
코드가 있을 때 그 중 어떤 부분이 function이고 그것의 기능이 무엇이고,
어떻게 쓰는지를 체득하는것이 중요하다.
